...
public class DetailServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			Todo todo = service.todoDetailView(index); // index번째 todo 없으면 null 반환
			
			// index 번째 todo가 존재하지 않을 경우 
			// -> 메인 페이지(/) redirect 후  
			// "해당 index 에 할 일이 존재하지 않습니다" alert 출력
			if(todo == null) {
			...
			String path = "/WEB-INF/views/detail.jsp"; // JSP 파일 경로(webapp 폴더 기준으로 작성)

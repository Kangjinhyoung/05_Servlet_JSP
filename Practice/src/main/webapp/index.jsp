...
<body>
  ...
  <pre>
    ...
    * src/main/webapp
      ...
      - 그래서 파일 경로, 요청 주소 작성 시 webapp 폴더가 기준이 된다
         ex) 주소 / : webapp의 index.jsp 요청
             ...
             css, js 파일 경로 ( ==요청주소): webapp 폴더 기준
    ...
    * scr/main/WEB-INF
      ...
      - 이럴거면 배포하는 webapp 폴더에 안넣으면 되는거 아닌가?
        -> 직접 해당 파일을 보여줄 수는 없지만 응답 화면 등의 웹 서비스를 
  ...
  <pre>
    ...
    - 존재하는 경우 
      -> "OOO님은 X번째 인덱스에 존재 합니다" 문자열을 request scope 객체에 세팅하여 
         /WEB-INF/views/result.jsp로 요청 위임해 결과 출력 (forward)

    - 존재하지 않은 경우
      -> "OOO님은 존재하지 않습니다" 문자열을 session scope에 세팅 후 
         "/" (메인 페이지 == index.jsp)로 redirect
  </pre>

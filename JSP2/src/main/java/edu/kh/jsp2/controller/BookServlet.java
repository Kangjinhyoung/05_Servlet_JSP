...
public class BookServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		bookList.add(new Book("Servlet이란", "철수", 40000));
		bookList.add(new Book("JSP란",       "유리", 80000));
		bookList.add(new Book("Spring 이란", "이슬", 200000));
		
		// bookList를 요청 위임된 JSP에서도 유지되어 사용할 수 있도록 request scope 객체에 속성으로 추가

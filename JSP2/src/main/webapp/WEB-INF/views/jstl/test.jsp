...
<body>
  ...
  <pre>
    - 원하는 scope 객체에 값을 세팅할 수 있는 태그 == 객체.setAttribute("key", value);
  ...
  <h5>application - num$ : ${applicationScope.num4}</h5>

  <hr>
  <h3>2. 변수 제거( == 속성 제거) - c:remove 태그</h3>
  <pre>
    - scope 객체에 세팅 된 속성을 제거하는 태그 == 객체.removeAttribute("key");
  ...
  <table>
    <tr>
      ...
      <%-- 모든 scope의 test2 제거 --%>
      <c:remove var="test2" /> <%-- scope 미작성 -> 전체 제거 --%>
  ...
  <hr>
  <h3>4. 연속된 조건문 - c:choose, c:when, c:otherwise 태그</h3>
  <pre>
    [c:choose 태그]
    ...
    * 주의 사항 * 
    바로 한 단계 아래에는 c:when, c:otherwise, 공백, jsp 주석만 작성 할 수 있고
    나머지는 모두 에러 발생!!
...
<hr>
<h3>5. 향상된 for문 + 추가 기능 - c:forEach 태그</h3>
<pre>
  [속성]
...
<ul>
  ...
  <li> <%-- 
        접두사 fn이 추가된 EL 구문을 사용해아 한다!! 

        fn(function, 기능) : 문자열, 컬렉션 관련 기능을 제공하는 JSTL
        
        ${fn:length(컬렉션 | 배열 | 문자열)}
        -> 컬렉션 | 배열 | 문자열의 길이 반환
        --%>
    nameList 길이(저장된 데이터 개수) : ${fn:length(nameList)}
  </li>
...
<ul><%-- varStatus : 현재 반복 상태와 관련된 정보를 제공하는 변수 --%>

...
<body>
  <h2>EL의 특징</h2>
  <pre>
    1. null을 빈칸("")으로 처리 + NullPointerException을 발생시키지 않음!!
  ...
  <ul>
    <li>
      JSP 표현식으로 str 파라미터 얻어오기 : <%= request.getParameter("str") %>    
    </li>
    <li>
  ...
  <h4>존재하지 않는 파라미터 얻어오기</h4>
   <ul>
    <li>
      JSP 표현식으로 temp 파라미터 얻어오기 : <%= request.getParameter("temp") %>    
    </li>
    <li>
  ...
  <ul> <%-- EL은 Java로 변환 되지만 문자열 비교는 == (비교 연산자) 사용 가능 --%>
    <li>${param.str == "abc"}</li>
    <%-- HTML 에서 얻어온 데이터는 모두 String 
    하지만 EL에서 연산 되는 자료형이 다를 경우 자동으로 자료형 변환(parsing) 진행해줌 --%>
  ...
  <ul>
    <li> <%-- check가 여러 개 전달된 경우 첫 번째 값 출력 --%>
      param.check : ${param.check}
    </li>
    <li> <%-- check라는 key를 가진 모든 값을 모아 String[]로 반환 --%>

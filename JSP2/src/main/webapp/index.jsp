...
<body>
  ...
  html : html, css, js 코드 작성 가능 + 정적 페이지(미리 만들어진 형태 그대로)
  jsp  : html, css, js, java, el, jstl 작성 가능 + 동적 페이지(요청에 따라 응답 화면 변함 가능)
  </pre>
  <hr>
  ...
  <h3>forward : 요청 위임</h3>
  <pre>
    - 클라이언트의 요청을 받은 Servlet / JSP가 직접 응답하는 것이 아닌다른 Servlet / JSP에 
      HttpServletRequest, HttpServletResponse 객체를 넘겨소(위임) 대신 응답하게 하는 것

    - 요청 위임시 RequestDispatcher(요청 발송자)를 이용
  ...
  <form action="/fr/forward">
    ...
    <button>포워드 확인하기</button>
  </form>

  <hr>
  <h3>redirect(재요청, 다른 Servelt 요청)</h3>
  <pre>
    - 클라이언트의 요청을 받은 Servlet / JSP에서 직접 응답하지 않고 
      다른 Servlet / JSP를 다시 요청 하는 것
                                                       (요청 받은 Servlet이 응답해줄 JSP가 없음)
    - 보통 Servlet 요청 처리 후 특정 JSP로 요청 위임해 결과 화면을 응답해 주는 것이 아닌 
    ...
    - redirect는 응답 화면의 주소가 처음 요청한 Servlet 주소가 아닌 다시 요청한 Servlet 주소로 변경된다
